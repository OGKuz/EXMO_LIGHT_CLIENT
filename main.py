...
class MyWindow(QtWidgets.QMainWindow):  
    ...
    def push_order(self):
        ...
        if self.ui.Price.text():
            ...
            price = 0
        send_order = self.driver.order_create(base=base, quote=quote, type=type, quantity=quantity, price=price)
        print (send_order)
        self.get_balance()
    ...
    def get_balance(self):
        try:
            ...
            df['balances'] = pd.to_numeric(df['balances'], downcast='float')
            df = df['balances'][df['balances']>0]
            df = df.to_dict()
            for _ in df.keys():
            ...
            self.set_line_balaces(balances['balances'])
